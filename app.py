...
def main():
    ...
    st.sidebar.title("Binary Clssification Web App")
    ...
    @st.cache(persist=True) # This decoder is to keep the data in cache    
    def load_data():
    	data = pd.read_csv('mushrooms.csv')
    	le = LabelEncoder() # instance of label encoder
    ...
    def split(df):
    	y = df['class'] # class column is our labels
    	x = df.drop(columns=['class']) # droping class columns and taking rest as features
    ...
    if classifier == 'Support Vector Machine (SVM)':
    	...
    	metrics = st.sidebar.multiselect("Metrics to plot?",('Confusion Matrix',
    ...
    if classifier == 'Random Forest Classifier':
    	st.sidebar.subheader("Model Hyperparameters")
    	n_estimators = st.sidebar.number_input("Number of trees in the forest", 100, 500,
    				 key='n_estimators')
    	max_depth = st.sidebar.number_input("Maximum depth of the tree", 1, 20, step=1,
    					key='max_depth')
    	bootstrap = st.sidebar.radio("Boostrap samples when building trees", ('True', 'False'),
    					key='bootstrap')
    ...
    # Checkbox
    st.sidebar.subheader("Have a look at the data")
